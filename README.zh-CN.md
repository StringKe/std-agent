...
![std-agent：22 个 AI CLI 工具的唯一事实来源](docs/assets/hero.png)
...
`stdagent` 是一个轻量的纯 Go CLI 工具。它把项目的 AI 配置维护在单一的 `.stdai/` 目录中作为唯一事实来源，再扩散到 **22 个 AI CLI 工具**，各工具的原生文件格式、frontmatter 方言和各种限制都已经替你处理好。
...
- **二十二个目标**：Claude Code、Codex、Cursor、GitHub Copilot、Windsurf/Devin、Gemini CLI、Aider、Cline、OpenCode、Roo Code、Crush、Amp、Warp、Factory、Continue.dev、Antigravity、Qwen Code、Pi、Kilo Code、Augment Code、Jules、Grok Build。
...
- **[docs/spec.md](docs/spec.md)**：完整规范，std-agent 标准 + 22 个工具的差异 + 转换策略
