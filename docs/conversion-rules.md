...
codex rules 全文 inline 到 AGENTS.md（Codex 项目级唯一指令机制，
project_doc_max_bytes 硬上限 32768）。不再拆分到 `.codex/` 子目录：项目级
`.codex/` 是官方 Team Config 配置目录（config.toml / rules/*.rules execpolicy /
skills/），memories 是 `~/.codex/memories/` 用户级自动记忆系统，均不承载
markdown 指令。总字节超预算由 budget root-file 检查 WARN，提醒用户精简。
