# std-ai Specification

stdagent 跨工具 AI 配置同步标准与转换规则的权威文档。本 spec 定义：

- std-ai 自身的源文件 schema（YAML frontmatter + Markdown）
...
6. targets / exclude_targets 内值必须是合法 target 名（11 个）
7. priority 在 enum 内
...
Anthropic 发起后开源）。std-ai 的 skill 直接对齐此标准。
...
| 字段 | 类型 | 转换策略 |
|---|---|---|
| `when_to_use` | string | std-ai 拼接到 description 末尾 |
...
std-ai SKILL.md 接受所有上述字段；每个 transformer 按 target 能力裁剪。
...
- Claude Code 还支持：`${CLAUDE_SKILL_DIR}/scripts/lint.sh` 绝对路径替换、`` !`cmd` `` 行内
  shell 注入；其他 target 不识别这两类语法，std-ai 转换器对 Claude Code 直传，对其他降级为
  普通文本
...
**std-ai 落地**：`.stdai/standards/rules/<name>.md`
...
**std-ai 落地**：`.stdai/standards/skills/<name>/`（package 形式）
...
**std-ai 落地**：`.stdai/standards/commands/<name>.md`
...
**std-ai 落地**：`.stdai/standards/references/<name>.md`

**典型用例**：架构概览、API 端点列表、术语表。

## Part 3: 11 个目标工具中的概念差异
...
```html
<!-- Generated by stdagent vX.Y.Z at <RFC3339>. Source: ... -->
```
...
1. 调 `https://api.github.com/repos/StringKe/std-ai/releases/latest` 拿 tag
2. 比对当前 `versionStr`，已最新且非 `--force` 时跳过
3. 下载平台对应归档：`std-ai_<ver>_<os>_<arch>.<tar.gz|zip>`
...
- [docs/targets/](targets/) 11 个目标工具调研
